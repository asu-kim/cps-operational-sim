...
def square_points(side):
    # Square centerline, counter-clockwise, starting on the lower-left corner.
    return [
        (0.0, 0.0),
        (side, 0.0),
        (side, side),
...
def write_model(out_path, csv_path, side, line_width, start_side):
    sx, sy, sth = read_start_pose(csv_path)
    pts = square_points(side)
    ...
    lines = []
    lines.append('<mujoco model="fixed_square_racetrack">')
    ...
    print("Ground truth type: square track")
    print(f"Side length={side:.4f} m")
...
def main():
    ...
    ap.add_argument("--side", type=float, default=0.2794)
